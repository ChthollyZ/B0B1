...
def main():
    ...
    parser.add_argument('--folder_b0', type=str, default=None, help='input b0 test image folder')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        # read image
        imgname, img_lq, img_gt, img_b0 = get_image_pair(args, path)  # image to HWC-BGR, float32
        ...
        # img_b0 = (img_b0 * 255.0).round().astype(np.uint8)  # float32 to uint8
        # img_b0 = img_b0[:h_old * args.scale, :w_old * args.scale, ...]  # crop gt
        # img_b0 = np.squeeze(img_b0)
        output = (output + img_b0)/2
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        img_b0 = cv2.imread(f'{args.folder_b0}/{imgname}_SwinIR{imgext}', cv2.IMREAD_COLOR).astype(np.float32) / 255.
        ...
        # img_gt = cv2.cvtColor(img_gt, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.

        # img_b0 = cv2.imread(f'{args.folder_b0}/{imgname}_SwinIR{imgext}', cv2.IMREAD_COLOR)
        # img_b0 = cv2.cvtColor(img_b0, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.

        # img_lq = cv2.imread(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}', cv2.IMREAD_COLOR)
        # img_lq = cv2.cvtColor(img_lq, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.
    ...
    return imgname, img_lq, img_gt, img_b0
